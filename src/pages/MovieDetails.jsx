...
import { FiCalendar, FiUser, FiFilm, FiGlobe, FiStar, FiPlay } from "react-icons/fi";

export default function MovieDetails() {
  const { id } = useParams();
  const { data, loading, error } = useFetch(() => get(`/api/movies/${id}`), [id]);
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="md:col-span-1">
            <div className="rounded-xl overflow-hidden shadow-lg border border-gray-200 hover:shadow-xl transition h-fit">
              {movie.poster ? (
                <img src={movie.poster} alt={movie.name} className="w-full h-auto object-cover max-w-xs" />
              ) : (
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">{movie.name}</h1>
            ...
            <div className="grid grid-cols-2 gap-3">
              {movie.language && (
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                  ...
                  <p className="text-gray-900 font-semibold text-sm">{movie.language}</p>
                </div>
              )}
              ...
              {movie.releaseDate && (
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                  ...
                  <p className="text-gray-900 font-semibold text-sm">{formatDate(movie.releaseDate)}</p>
                </div>
              )}
              ...
              {movie.director && (
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                  ...
                  <p className="text-gray-900 font-semibold text-sm">{movie.director}</p>
                </div>
              )}
              ...
              {movie.genres?.length > 0 && (
                <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                  ...
                  <p className="text-gray-900 font-semibold text-sm">{movie.genres.join(", ")}</p>
                </div>
              )}
            </div>
            ...
            {movie.description && (
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                <h3 className="text-base font-semibold text-gray-900 mb-2">Synopsis</h3>
                <p className="text-gray-700 leading-relaxed text-sm">{movie.description}</p>
              </div>
            )}
            ...
            {movie.cast?.length > 0 && (
              <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                <h3 className="text-base font-semibold text-gray-900 mb-2">Cast</h3>
                <div className="flex flex-wrap gap-2">
                  {movie.cast.map((actor, idx) => (
                    <span key={idx} className="px-3 py-1 bg-gray-100 text-gray-800 text-xs rounded-md hover:bg-gray-200 transition border border-gray-300">
                      {actor}
                    </span>
                  ))}
                </div>
