...
export default function TheaterDetail() {
  ...
  const { data, loading, error } = useFetch(() => get(`/api/theaters/${id}`), [id]);
  ...
  if (!theater) return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-b from-gray-50 to-white px-4">
      <div className="bg-gray-100 border border-gray-300 rounded-xl p-8 text-center">
        <p className="text-gray-700 font-semibold text-lg">Theater not found</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-linear-to-b from-white to-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm hover:shadow-md transition mb-10">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">{theater.name}</h1>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-gray-900 font-semibold">{theater.address}, {theater.city}</p>
                {theater.pincode && <p className="text-gray-600 text-sm">{theater.pincode}</p>}
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm text-gray-600 font-medium mb-1">Screens</p>
                <p className="text-gray-900 font-semibold text-lg">{theater.totalScreens}</p>
              </div>
            </div>
            ...
            {theater.description && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-700 text-sm line-clamp-2">{theater.description}</p>
                </div>
              </div>
            )}
          </div>
